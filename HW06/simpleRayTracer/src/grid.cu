...
__device__ colour_t gridTrace(const grid_t grid,
  ...
  while(rayID<Nrays && Nrays<p_maxNrays){
	...
	for(unsigned int j=0; j < Nlights; j++){
	  ...
	  int shadowShapeID = -1;
	 __device__ gridRayIntersectionSearch(lightRay, Nshapes, shapes, grid, &tshadow, &shadowShapeID);
...
void gridPopulate(grid_t *grid, int Nshapes, shape_t *shapes){
  ...
  if(grid->boxContents){
    ...
    free(grid->boxStarts);
    cudaFree(c_grid->boxContents);
    cudaFree(c_grid->boxStarts);
  }
...
//cudaMalloc(&scene->c_lights, Nlights * sizeof(lights_t));
cudaMalloc(&c_grid->boxContents, Nentries * sizeof(int));  
  cudaMemcpy(c_grid->boxContents, grid->boxContents, Nentries * sizeof(int),cudaMemcpyHostToDevice);  

cudaMalloc(&c_grid->boxStarts, (Nboxes + 1) * sizeof(int));
  cudaMemcpy(c_grid->boxContents, grid->boxContents, (Nboxes + 1) * sizeof(int),cudaMemcpyHostToDevice);
