...
scene_t *sceneSetup(){
  ...
  cudaMalloc(&scene->c_materials, Nmaterials * sizeof(material_t));     
     cudaMemcpy(scene->c_materials, materials, Nmaterials * sizeof(materials_t), cudaMemcpyHostToDevice);    
cudaMalloc(&scene->c_lights, Nlights * sizeof(lights_t));
 cudaMemcpy(scene->c_lights, lights, Nlights * sizeof(lights_t), cud\
aMemcpyHostToDevice);
  cudaMalloc(&scene->grid.c_bboxes, grid->NI*grid->NJ*grid->NK * sizeof(bbox_t)
   cudaMemcpy(scene->grid.c_bboxes,bboxes, grid->NI*grid->NJ*grid->NK * sizeof(bbox_t), cudaMemcpyHostToDevice);
